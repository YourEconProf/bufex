def create_new_game(game_file):
	...
	#pdb.set_trace()
	roster = pd.read_csv(roster_file_name)
	roster.columns = [['All','All','All','All'],['Name','Badge','Code','Deposit']]
	#roster = roster.set_index(roster.All.Badge)
...
def import_trading_data(game_file,import_trades):
	...
	closed['Settled']=( closed['Cmdty'].isin(close_settle_symbols) | ~np.any(np.isnan(closed[['BuyPrice','SellPrice']]),1) )
	...
	for idx in closed.index:
		for pString in PriceStrings:
			if np.isnan(closed[pString][idx]): 
			    closed.loc[idx,pString]=contracts['Price'][closed['Cmdty'][idx]]
	...
	# Settled Profit
	new_roster[session_name,'ClsBal']=new_roster['All','Deposit'] + new_roster.xs('ClsPrft',level=1,axis=1).sum(axis=1)
	new_roster[session_name,'MTMBal']=new_roster[session_name,'ClsBal']+new_roster[session_name,'MTMPrft']-new_roster[session_name,'Margin']
...
def expand_transactions_old(trans):
	import pandas as pd
	import pdb
	...
	#trans.drop('Badge',axis=1,inplace=True)
	#trans.drop('Price',axis=1,inplace=True)
	#del trans['Badge']
	#del trans['Price']
	...
	for item in trans.index: # loops over rows in trans
		counter = 1
		for i in range(abs(trans['Qty'][item])): # loops over number of contracts in this row
...
def expand_transactions(trans):
	...
	# combine any rows that need combining
	trans = trans.groupby(['Badge','Cmdty','Price'])['Qty'].sum()
	trans = trans.reset_index()
	...
	for item in trans.index: # loops over rows in trans
		counter = 1
		for i in range(abs(trans['Qty'][item])): # loops over number of contracts in this row
			out=out.append(trans.iloc[item],ignore_index=True)
			out.loc[max(out.index),'Qty']=counter
			counter = counter + 1
	#pdb.set_trace()
	out=out.iloc[1:]
...
def print_trades(buys,sells,game_file,session_name):
	...
	all=all.sort_values(by=['Code','Cmdty'])
	#all.to_csv(game_file+session_name+"_trading.csv",index=False,columns=['Code','Qty','Cmdty','Price'])
	all.to_html(open(game_file+"_"+session_name+"_trading.html",'w'),index=False,columns=['Code','Qty','Cmdty','Price'])
...
def print_open_positions(open_positions,game_file,session_name):
	...
	all=all.sort_values(by=['Code','Cmdty'])
	all.to_html(open(game_file+"_"+session_name+"_open_positions.html",'w'),index=False,columns=['Code','Cmdty','BuyPrice','Qty','SellPrice'])
